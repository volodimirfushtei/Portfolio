...
class ErrorBoundary extends Component {
  ...
  playEntranceAnimation = () => {
    ...
    const titleLines = this.contentRef.current.querySelectorAll(
      `.${s.headingLine} span`,
    )
    ...
    tl.from(
      `.${s.eyebrow}, .${s.iconRow}, .${s.desc}, .${s.errorCard}, .${s.actions}, .${s.support}, .${s.statusBar}`,
      {
        opacity: 0,
        y: 20,
        stagger: 0.1,
      ...
      <div className={s.container}>
        ...
        <div className={s.content} ref={this.contentRef}>
          <div className={s.header}>
            ...
            <p className={s.desc}>
              Our high-integrity error boundary has intercepted a process
              anomaly to ensure application stability.
            </p>
          </div>
          ...
          <div className={s.statusBar}>
            ...
            <span>
              Boundary Monitor Active · {new Date().toLocaleTimeString()}
            </span>
          </div>
        </div>
